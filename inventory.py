# inventory module; contains info about
# items and stock. DO NOT MODIFY THIS MODULE!
...
class Inventory:
    ...
    async def get_number_of_items(self):
        # returns number of unique items in the restaurants catalogue.
    ...
    async def get_stock(self, item_id):
        # returns the stock count for 'item_id'
        await asyncio.sleep(2)
        if item_id not in self.stock:
            raise ValueError(
                f"No item with id: {item_id} exists in the inventory.")
    ...
    async def decrement_stock(self, item_id):
        # decrements by 1 the stock count for 'item_id'
        if item_id not in self.stock:
            raise ValueError(
                f"No item with id: {item_id} exists in the inventory.")
    ...
    async def get_item(self, item_id):
        # returns a dictionary with item details for 'item_id'
        await asyncio.sleep(1)
